...
public class LAppModel extends CubismUserModel {
    ...
    /**
     * 모델의 갱신 처리. 모델의 파라미터에서 그리기 상태를 결정한다
     */
    public void update() {
...
//         前回セーブされた状態をロード
        ...
        if (expressionManager != null) {
         ...
         // 드래그 추종 기능
         // 드래그를 통한 얼굴 방향 조정
        model.addParameterValue(idParamAngleX, dragX * 30); // -30에서 30의 값을 더하다
        ...
        // Lip Sync Setting
        if (lipSync) {
            // 실시간으로 립싱크를 하는 경우, 시스템에서 음량을 취득하여 0~1의 범위에서 값을 입력합니다
            float value = 0.0f;
     ...
     * 콜백 함수가 전달되지 않았을 경우에 그것을 null로서 동메소드를 호출한다.
     *
     * @param group 모션그룹명
     * @param number 그룹내 번호
     * @param priority 우선도
     * @return 시작한 모션의 식별번호를 반환한다. 개별 모션이 종료되었는지 여부를 판별하는 isFinished() 인수로 사용한다. 시작이 안 될 때는 '-1'
     */
     ...
     * @param number 그룹내 번호
     * @param priority 우선도
     * @param onFinishedMotionHandler 모션 재생 종료 시 호출되는 콜백 함수. null일 경우 호출되지 않는다.
     * @return 시작한 모션의 식별번호를 반환한다. 개별 모션이 종료되었는지 여부를 판정하는 is Finished() 인수로 사용한다. 시작이 안 될 때는 '-1'
     */
    ...
    /**
     * 랜덤으로 선택된 모션의 재생을 시작한다.
     * 콜백 함수가 전달되지 않았을 경우에 그것을 null로서 동메소드를 호출한다.
     *
     * @param group 모션그룹명
     * @param priority 우선도
     * @return 시작한 모션 식별 번호. 개별 모션이 종료되었는지 여부를 판정하는 is Finished() 인수로 사용한다. 시작이 안 될 때는 '-1'
     */
     ...
     * @param priority 우선도
     * @param onFinishedMotionHandler 모션 재생 종료 시 호출되는 콜백 함수. null일 경우 호출되지 않는다.
     * @return 시작한 모션의 식별번호를 반환한다. 개별 모션이 종료되었는지 여부를 판정하는 is Finished() 인수로 사용한다. 개시할 수 없을 때는 -1
     */
    ...
    /**
     * 당첨 판정 테스트
     * 지정 ID의 정점 리스트에서 직사각형을 계산하여 좌표가 직사각형 범위 내인지 판정하다
     *
     * @paramhitAreaName당 판정을 테스트하는 대상 ID
     * @param x 판정을 하는 x 좌표
     * @paramy 판정을 하는 y 좌표
     * @return 맞다면 true
     */
     ...
     * .moc3 파일의 정합성을 체크한다.
     *
     * @parammocFileName MOC3 파일명
     * @return MOC3에 정합성이 있는지 여부. 정합성이 있으면 true.
     */
    ...
    /**
     * 모션 데이터를 그룹명에서 일괄로 로드한다.
     * 모션 데이터의 이름은 Model Setting 에서 취득한다.
     *
